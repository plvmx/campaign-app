...
2. Run `add_campaigns_rls.sql` if campaigns table still has no RLS
3. Run `improve_app_settings_rls.sql` and `improve_campaign_messages_rls.sql` if those tables need policy updates
4. Run `add_state_leaders_mobile_index.sql` for performance
