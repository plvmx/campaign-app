...
**Amendment (2026-09-14):** the above is true of List `[1]`, but List `[2]` (`/wayoflife-responder/`) was initially believed — this investigation and prior discovery work alike — to be the sole use of that list, unlike List `[1]`. A same-day follow-up audit of every AC tag actually in use found this to be only mostly true: a second, distinct tag (`[6]` "FORM: TWOL Video: Requested") also lands on List `[2]`, so the routing decision below is deliberately keyed on `source_list_id === '2'` itself, not on the tag — it doesn't matter which List-2 source a contact came through, none of them are self-registrations. Its submissions are also not self-registrations at all (a TWOL presenter reporting on someone they presented to) and have no Postcode field. `registry.registrants` originally included them anyway; they're now routed to a separate `registry.twol_respondents` table instead — a deliberate, narrow exception to this section's "never key off List ID" rule. (`[6]` was subsequently excluded entirely rather than kept in `twol_respondents` — see the amendment below and `OPERATIONS.md`'s follow-up entry.) See `scripts/create_registry_twol_respondents_table.sql` and `OPERATIONS.md`'s 2026-09-14 entries for the full detail.

**Amendment 2 (2026-09-14, same day):** the same tag audit also found two more tags with no genuine registration-funnel signal behind them — `[40]`/`[41]` "Mobilise - Make a Donation: Form completed" (financial-intent-adjacent, same sensitivity category as the already-excluded `[12]`/`[13]` and List 5) and `[8]`/`[9]` "TWOL Explore More: Requested" (near-blank contacts) — plus reconsidered `[6]` itself (a video-request click, not a response report). All three were added to `EXCLUDED_SOURCE_TAG_IDS` in `lib/registryPipeline/tagExclusion.ts`, on the same footing as `[11]`'s pre-existing MailChimp exclusion in Section 3.4/6.2 below — excluded from the registry entirely, not routed anywhere. See `OPERATIONS.md`'s follow-up entry.
