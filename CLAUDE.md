...
- **Registrations reload from Lorraine's spreadsheet** — built 2026-09-08 (`scripts/reload_registrants_from_csv.ts` + `lib/registryPipeline/{csvParse,csvRegistrantTransform}.ts`, both unit tested), **not yet run against the live database** — see `OPERATIONS.md`'s "built, not yet run" entry for the exact order of operations (backup, migration, truncate, dry run, apply, redeploy `ac-sync`).
