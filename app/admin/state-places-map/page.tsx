...
export default function StatePlacesMapPage() {
  ...
  useEffect(() => {
    ...
    Promise.resolve()
      .then(() => {
        ...
        setMapError(null);
        return getStatePlacesMapData({ state: selectedState || undefined });
      })
      .then(result => {
        if (cancelled) return;
      ...
      .catch((err: unknown) => {
        if (cancelled) return;
        setMapError(getErrorMessage(err, 'Failed to load state places for the map'));
      })
      .finally(() => {
    ...
    <MobileLayout>
      ...
      <div className="flex h-[calc(100dvh-var(--pwa-banner-height,0px)-4rem-5rem)] flex-col p-4">
        <div className="mb-3 flex items-start justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">State Places Map</h1>
            <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
              Every state place plotted on an interactive map of Australia
            </p>
          </div>
          <button
        ...
        <div className="relative flex-1 overflow-hidden rounded-lg border-2 border-gray-800 dark:border-gray-600">
          {isLoadingMap && (
            ...
            <div className="absolute inset-0 z-[1100] flex flex-col items-center justify-center gap-3 bg-white/80 px-6 text-center dark:bg-gray-900/80">
              <LoadingSpinner text="Please wait — locating places on the map" />
              <p className="max-w-sm text-xs text-gray-600 dark:text-gray-400">
                This can take a moment the first time, while new place locations are looked up. Subsequent loads are instant.
              </p>
            </div>
          )}
