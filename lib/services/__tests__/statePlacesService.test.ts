...
describe('createStatePlace', () => {
  it('inserts the place', async () => {
    ...
    mockFrom.mockReturnValue(builder);
    await createStatePlace({ state: 'VIC', place: 'Melbourne', location: 'Melbourne' });
    expect(builder.insert).toHaveBeenCalledWith([{ state: 'VIC', place: 'Melbourne', location: 'Melbourne' }]);
  });
  ...
  it('raises a friendly error on a duplicate state+place (23505)', async () => {
    ...
    await expect(createStatePlace({ state: 'VIC', place: 'Melbourne', location: 'Melbourne' })).rejects.toThrow(
  ...
  it('rethrows other errors as-is', async () => {
    const error = { code: '500', message: 'boom' };
    mockFrom.mockReturnValue(makeQueryBuilder({ data: null, error }));
    await expect(createStatePlace({ state: 'VIC', place: 'Melbourne', location: 'Melbourne' })).rejects.toEqual(error);
  });
});
...
describe('updateStatePlace', () => {
  it('updates by id', async () => {
    const builder = makeQueryBuilder({ data: null, error: null });
    mockFrom.mockReturnValue(builder);
    await updateStatePlace('p1', { state: 'VIC', place: 'Geelong', location: 'Geelong' });
    expect(builder.update).toHaveBeenCalledWith({ state: 'VIC', place: 'Geelong', location: 'Geelong' });
  ...
  it('throws on error', async () => {
    const error = { code: '500', message: 'boom' };
    mockFrom.mockReturnValue(makeQueryBuilder({ data: null, error }));
    await expect(updateStatePlace('p1', { state: 'VIC', place: 'Geelong', location: 'Geelong' })).rejects.toEqual(error);
  });
});
